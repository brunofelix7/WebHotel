package br.unipe.hotel.controller;

import java.util.List;
import javax.transaction.SystemException;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.validation.Errors;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.ModelAttribute;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.servlet.ModelAndView;
import br.unipe.hotel.data.CategoriaData;
import br.unipe.hotel.entity.Categoria;
import br.unipe.hotel.service.CategoriaService;

@Controller
@RequestMapping(value = "/categorias")
public class CategoriaController {

	private static final String CADASTRAR_VIEW = "/categoria/CadastrarCategoria";
	private static final String ATUALIZAR_VIEW = "/categoria/AtualizarCategoria";
	private static final String LISTAR_VIEW = "/categoria/ListarCategorias";
	
	@Autowired
	private CategoriaService categoriaService;
	
	@Autowired
	private CategoriaData categoriaData;
	
	
	//	1. REDIRECIONA PARA PÁGINA DE CADASTRO
	@RequestMapping(value = "/novo", method = RequestMethod.GET)
	public ModelAndView novo(ModelMap modelMap) {
		ModelAndView mv = new ModelAndView(CADASTRAR_VIEW);
		Categoria categoria = new Categoria();
		mv.addObject("categoria", categoria);
		return mv;
	}
	
	//	2. SALVAR
	@RequestMapping(value = "/salvar", method = RequestMethod.POST)
	public ModelAndView salvar(@ModelAttribute(value = "categoria") @Validated Categoria categoria, Errors errors, ModelMap modelMap){
		ModelAndView mv = new ModelAndView(CADASTRAR_VIEW);
		modelMap.addAttribute("categoria", categoria);
		try{
			if(errors.hasErrors()){
				return mv;
			}else{
				categoriaService.salvar(categoria);
				mv.addObject("mensagem", "Salvo com sucesso!");
			}
		}catch(SystemException e){
			e.printStackTrace();
		}
		return mv;
	}
	
	//	3. ATUALIZAR
	@RequestMapping(value = "/alterar", method = RequestMethod.POST)
	public ModelAndView salvarAlteracoes(@ModelAttribute(value = "categoria") @Validated Categoria categoria, Errors errors, ModelMap modelMap){
		ModelAndView mv = new ModelAndView(ATUALIZAR_VIEW);
		modelMap.addAttribute("categoria", categoria);
		try{
			if(errors.hasErrors()){
				return mv;
			}else{
				categoriaService.atualizar(categoria);
				mv.addObject("mensagem", "Dados atualizados com sucesso!");
			}
		}catch(SystemException e){
			e.printStackTrace();
		}
		return mv;
	}
	
	//	4. REDIRECIONA PARA PÁGINA DE ATUALIZAÇÃO
	@RequestMapping(value = "/atualizar/{id}", method=RequestMethod.GET)
	public String atualizar(@PathVariable(value="id") Long id, ModelMap modelMap){
		Categoria categoria = categoriaService.buscarPorId(id);
		modelMap.addAttribute("categoria", categoria);
		return ATUALIZAR_VIEW;
	}
	
	//	5. EXCLUIR
	@RequestMapping(value = "/excluir/{id}", method=RequestMethod.GET)
	public String excluir(@PathVariable(value="id") Long id, ModelMap modelMap){
		try{
			Categoria categoria = categoriaService.buscarPorId(id);
			categoriaService.remover(categoria);
		}catch (SystemException e) {
			e.printStackTrace();
		}
		return "redirect:/categorias";
	}
	
	//	6. LISTAR
	@RequestMapping(method = RequestMethod.GET)
	public String listar(ModelMap modelMap) {
		List<Categoria> categorias = categoriaService.listar();
		modelMap.addAttribute("categorias", categorias);
		return LISTAR_VIEW;
	}
	
	//	7. PESQUISA
	@RequestMapping(value = "/filtrar", method = RequestMethod.GET)
	public String pesquisar(@ModelAttribute(value="nome") String nome, ModelMap modelMap){
		List<Categoria> categorias = categoriaData.findByNomeContaining(nome);
		modelMap.addAttribute("categorias", categorias);
		return LISTAR_VIEW;
	}
		
}
